...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; ) {
    const objEntries = Object.entries(objects[i]);
    for (let j = 0; j < objEntries.length; ) {
      if (Object.hasOwn(res, objEntries[j][0])) {
        res[objEntries[j][0]] += parseInt([objEntries[j][1]], 10);
      } else {
        res[objEntries[j][0]] = parseInt([objEntries[j][1]], 10);
      }
      j += 1;
...
function compareObjects(obj1, obj2) {
  ...
  for (let i = 0; i < arr1.length; ) {
    if (obj1[arr1[i]] !== obj2[arr1[i]]) return false;
    i += 1;
  }
...
function makeWord(lettersObject) {
  ...
  for (let key = 0; key < keys.length; ) {
    for (let i = 0; i < lettersObject[keys[key]].length; ) {
      wordArr[lettersObject[keys[key]][i]] = keys[key];
      i += 1;
    }
    key += 1;
  }
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; ) {
    if (queue[i] === 100) {
      if (!Object.hasOwn(change, 25) || change[25] < 1) return false;
      if (Object.hasOwn(change, 50)) {
        if (change[50] >= 1) {
          change[50] -= 1;
          change[25] -= 1;
        } else if (change[25] > 2) {
    ...
    if (queue[i] === 50) {
      if (!Object.hasOwn(change, 25)) return false;
      if (change[25] < 1) return false;
      change[25] -= 1;
    }

    if (Object.hasOwn(change, queue[i])) {
      change[queue[i]] += 1;
    } else {
      change[queue[i]] = 1;
    }
...
function fromJSON(proto, json) {
  ...
  return Object.assign(res, JSON.parse(json));
}
...
function group(array, keySelector, valueSelector) {
  ...
  keys.forEach((key) => {
    ...
    const secondValues = values.map(valueSelector);

    mapped.set(key, secondValues);
  });
  return mapped;
}
