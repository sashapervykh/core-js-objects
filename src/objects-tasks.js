...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; ) {
    arr[i] = Object.entries(objects[i]);
    i += 1;
  }
  arr = arr.flat();
...
function makeWord(lettersObject) {
  const res = [];
  res.length = Object.values(lettersObject).flat().length;
  Object.keys(lettersObject).forEach((key) => {
    lettersObject[key].forEach((elem, index) => {
      res[lettersObject[key][index]] = key;
    });
  });
  return res.join('');
}
...
function sellTickets(queue) {
  ...
  function checkChange(money) {
    if (money === 100) {
      if (obj[50] < 1 && obj[25] < 1) return false;
      if (obj[25] < 3) return false;
      if (obj[25] > 2) {
        obj[100] += 1;
        obj[25] -= 3;
      } else {
  ...
  queue.forEach((elem, index) => {
    results[index] = checkChange(elem);
  });
...
function sortCitiesArray(arr) {
  const properties = Array.from(
    new Set(arr.map((elem) => elem.country))
  ).sort();
  const cities = [];
  cities.length = properties.length;
  arr.forEach((elem) => {
    const index = properties.indexOf(elem.country);
    if (cities[index] === undefined) {
      cities[index] = [elem.city];
    } else {
      cities[index][cities[index].length] = elem.city;
    }
  });

  cities.forEach((elem) => elem.sort());
  ...
  while (j < cities.length) {
    let i = 0;
    while (i < cities[j].length) {
      res[k] = { country: properties[j], city: cities[j][i] };
      i += 1;
      k += 1;
    }
    j += 1;
...
function group(array, keySelector, valueSelector) {
  const properties = Array.from(
    new Set(array.map((elem) => keySelector(elem)))
  );
  ...
  array.forEach((elem) => {
    const index = properties.indexOf(keySelector(elem));

    if (cities[index] === undefined) {
      cities[index] = [valueSelector(elem)];
    } else {
      cities[index][cities[index].length] = valueSelector(elem);
    }
  });

  const res = new Map();
  cities.forEach((elem, index) => {
    res.set(properties[index], elem);
  });
  return res;
}
